...
export default function ExploreBoyslist({ onBack, onSelectProfile, onOpenAdmin }: Props) {
  ...
  const [searchQuery, setSearchQuery] = useState(savedBoysSearch);
  ...
  // Restore scroll
  useEffect(() => {
    if (savedBoysScroll > 0 && containerRef.current) {
      setTimeout(() => {
        if (containerRef.current) {
          containerRef.current.scrollTop = savedBoysScroll;
        }
      }, 100);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      savedBoysScroll = container.scrollTop;
    };
 ...
 const loadProfiles = async (pageNum: number) => {
    try {
      ...
      console.log(`📡 Loading boys page ${pageNum} (items ${from}-${to})`);
      ...
      console.log(`✅ Loaded ${shuffledData.length} boys profiles`);
      ...
      console.error('Error loading boys:', error);
    } finally {
      setLoading(false);
